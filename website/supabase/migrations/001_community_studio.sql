...
language sql
stable
as $$ select coalesce(auth.jwt() -> 'amr', '[]'::jsonb) ? 'mfa' $$;
...
using (bucket_id = 'design-assets' and owner_id = public.request_user_id() and public.request_has_mfa())
with check (bucket_id = 'design-assets' and owner_id = public.request_user_id() and public.request_has_mfa());
